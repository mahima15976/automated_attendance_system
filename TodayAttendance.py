...
class Ui_TodayAttendance(object):
    def studentdetails(self):
        try:
          database = DBConnection.getConnection()
          cursor=database.cursor()
          cursor.execute("SELECT dt,rollno,name,sts FROM attendance")
          row = cursor.fetchall()
    ...
    def retranslateUi(self, Dialog):
        ...
        Dialog.setWindowTitle(_translate("Dialog", "Today Attendance"))
        item = self.tableWidget.horizontalHeaderItem(0)
        item.setText(_translate("Dialog", "Date"))
        item = self.tableWidget.horizontalHeaderItem(1)
        item.setText(_translate("Dialog", "Roll No."))
        item = self.tableWidget.horizontalHeaderItem(2)
        item.setText(_translate("Dialog", "StudentName"))
